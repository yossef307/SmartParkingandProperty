...
import { MapPin, Bed, Bath, Square, Car, Heart, Share2, Star, Loader2, CheckCircle2, AlertCircle } from 'lucide-react';
...
const PropertyDetails = () => {
    ...
    // --- الإعدادات الثابتة (تطابق قاعدة بيانات SQL Server) ---
    const propertyId = 1;
    const userId = 1;
        ...
        try {
            if (!isParkingEnabled) {
                ...
                if (!spots || !Array.isArray(spots)) {
                    throw new Error("لم يتم استلام بيانات الركنات من السيرفر.");
                }
                ...
                if (availableInZone) {
                    ...
                    alert(`عذراً، لا توجد أماكن متاحة حالياً في المنطقة (${selectedZone}) لهذا العقار.`);
                }
            } else {
                // إلغاء الحجز من خلال ID الركنة المرتبطة
                if (reservedSpot) {
            ...
            <main className="max-w-[1400px] mx-auto px-6 lg:px-12 py-8">
                ...
                <div className="grid grid-cols-4 grid-rows-2 gap-4 h-[500px] mb-12 rounded-[40px] overflow-hidden shadow-2xl shadow-blue-900/5">
                    <div className="col-span-2 row-span-2 relative group">
                        ...
                        <div className="absolute top-6 right-6 bg-white/90 backdrop-blur px-4 py-2 rounded-xl font-bold text-[10px] text-blue-600 shadow-sm">
                            RealPark System - IoT Connected
                        </div>
                    </div>
                    <div className="col-span-1 row-span-1 overflow-hidden">
                        <img src="https://images.unsplash.com/photo-1613977257363-707ba9348227" alt="Int" className="w-full h-full object-cover hover:scale-110 transition-duration-500" />
                    </div>
                    <div className="col-span-1 row-span-1 overflow-hidden">
                        <img src="https://images.unsplash.com/photo-1512917774080-9991f1c4c750" alt="Pool" className="w-full h-full object-cover hover:scale-110 transition-duration-500" />
                    </div>
                    <div className="col-span-2 row-span-1 relative group overflow-hidden">
                        <img src="https://images.unsplash.com/photo-1600585154340-be6161a56a0c" alt="Kitchen" className="w-full h-full object-cover group-hover:scale-105 transition-duration-700" />
                        <button className="absolute bottom-6 left-6 bg-slate-900/90 backdrop-blur-md text-white px-8 py-3 rounded-2xl font-bold text-xs hover:bg-blue-600 shadow-lg">
                            استكشاف 24 صورة
                        </button>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    <div className="lg:col-span-2">
                        <div className="flex flex-col gap-2 mb-6">
                            ...
                            <h1 className="text-5xl font-black text-slate-900 leading-tight">فيلا مودرن <span className="text-blue-600 underline decoration-blue-100 decoration-8 underline-offset-8">بيفرلي هيلز</span></h1>
                        </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-12">
                            ...
                            <FeatureItem
                                icon={<Car />}
                                label={isParkingEnabled ? `ركنة ${reservedSpot?.spotNumber}` : "ركن ذكي"}
                                highlight={isParkingEnabled}
                            />
                        </div>

                        {/* RealPark Info Section */}
                        <div className="bg-gradient-to-l from-blue-50/50 to-transparent p-8 rounded-[35px] border border-blue-50 mb-10">
                            ...
                            <p className="text-blue-800/70 text-md leading-relaxed font-medium">
                                من خلال دمج تقنيات IoT، نضمن لك مكاناً مخصصاً لسيارتك يتم فتحه تلقائياً عبر التطبيق عند اقترابك من العقار، مما يلغي عناء البحث عن ركنة.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-1">
                        <div className="sticky top-8 bg-white border border-blue-50 rounded-[40px] p-8 shadow-2xl shadow-blue-900/5">
                                ...
                                <div className="flex gap-2">
                                    {['A', 'B', 'C'].map((zone) => (
                                        <button
                                            ...
                                            className={`flex-1 py-4 rounded-2xl text-[11px] font-black transition-all ${selectedZone === zone ? 'bg-blue-600 text-white shadow-lg' : 'bg-white text-gray-400 border border-gray-100 hover:border-blue-200'}`}
                                        >
                            ...
                            <div className={`p-5 rounded-[25px] border-2 transition-all mb-8 flex justify-between items-center ${isParkingEnabled ? 'bg-blue-600 border-blue-500' : 'bg-blue-50/30 border-blue-100/50'}`}>
                                ...
                                >
                                    <div className={`absolute w-5 h-5 rounded-full top-1 transition-all duration-300 ${isParkingEnabled ? 'left-1 bg-white' : 'right-1 bg-white'} flex items-center justify-center shadow-md`}>
                            ...
                            <div className="space-y-4 mb-8 bg-slate-50/50 p-6 rounded-3xl border border-dashed border-slate-200">
                                <div className="flex justify-between text-xs font-bold text-slate-500">
                                    <span>الإقامة ({nights} ليالي)</span>
                                    <span>${villaBasePrice.toLocaleString()}</span>
                                </div>
                                {isParkingEnabled && (
                                    <div className="flex justify-between text-xs font-bold text-blue-600">
                                        <span>إضافة الركن (Zone {selectedZone})</span>
                                        <span>+ ${parkingPrice.toLocaleString()}</span>
                                    </div>
                                )}
                                <div className="flex justify-between text-xs font-bold text-slate-500">
                            ...
                            <button
                                ...
                                className={`w-full py-5 rounded-[22px] font-black text-lg transition-all active:scale-95 ${nights > 0 ? 'bg-slate-900 text-white hover:bg-blue-700 shadow-2xl shadow-blue-200' : 'bg-gray-100 text-gray-400'}`}
                            >
                                {isSubmitting ? <Loader2 className="animate-spin mx-auto" /> : 'تأكيد الحجز والدفع'}
                            </button>
                        </div>
                    </div>
                </div>
...
// --- المكونات المساعدة لتحسين الكود والقراءة ---
