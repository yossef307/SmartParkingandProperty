...
const AdminReports = () => {
    ...
    useEffect(() => {
        const fetchReportData = async () => {
            try {
                // ✅ تم تحديث البورت إلى 7048 بناءً على تشغيل السيرفر الحالي
                const response = await axios.get('https://localhost:7048/api/Reports/financial-summary');
                setReportData(response.data);
                setError(null);
            } catch (error) {
                console.error("Error fetching financial reports:", error);
                setError("Could not connect to the server. Please ensure Backend is running on port 7048.");
            } finally {
                setLoading(false);
            }
        };
